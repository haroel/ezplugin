...
export class PluginBridgeCore {
    ...
    */
    excute(pluginName: string, action: string, params: string | object, isInUITheard: boolean, callback?: PluginCallbackFunction): number {
        try {
            let len = arguments.length;
            if (len < 2) {
                throw new Error("[PluginCore]->excute :参数数不对，至少应有2个参数！");
            }
            let checkParamType = function (typeDefine, ...rest) {
                for (let i = 1; i < arguments.length; i++) {
                    if (typeDefine.indexOf(typeof arguments[i]) >= 0) {
                        return arguments[i];
                    }
                }
                return null;
            };
            let _callback = checkParamType('function', callback, params, isInUITheard);
            let _params = checkParamType('string|number', params, callback, isInUITheard);
            if (!_params) {
                _params = checkParamType('object', params, callback, isInUITheard);
            ...
            let _callId = this.exchangeCallbackID(_callback);
            if (cc.sys.isNative) {
                switch (cc.sys.os) {
                    case cc.sys.OS_ANDROID: {
                        let _isInUITheard = checkParamType('boolean', isInUITheard, _params, callback) || false;
                        let callName = 'excute';
                        if (_isInUITheard) {
    ...
    */
    ...
    nativeEventHandler(pluginName: string, event: string, params: string) {
        try {
            ...
            hascall || trace(`${pluginName} has nothing function to call! ${event}`);
        } catch (e) {
