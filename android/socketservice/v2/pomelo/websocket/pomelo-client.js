...
JavaWebsocket.prototype.close = function () {
    cc.log("JavaWebsocket ->call close ")
    this.readyState = WS_CLOSING;
    let ret = jsb.reflection.callStaticMethod("sockets/WebSocketClient",
        "disconnect", "()V");
    if (ret) {
        cc.log(ret);
    }
...
window["wshandler"] = {
    ...
    event: function (event, strdata) {
        cc.log("[pomelo-client] receive event: ", event);
        switch (event) {
            case "onopen": {
                if (this.websocket) {
            ...
            case "heartBeatTimeout": {
                if (this.websocket) {
                    this.websocket.heartBeatTimeout({ target: this.websocket })
                } else {
                    cc.error(" websocket is null!")
                }
                break;
