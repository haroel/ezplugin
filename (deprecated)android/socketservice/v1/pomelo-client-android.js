...
let JavaWebsocket = function (wsurl) {
    ...
    jsb.reflection.callStaticMethod("sockets/WebSocketClient",
        "createWS", "(Ljava/lang/String;)V", wsurl);
    window["wshandler"]["websocket"] = this;
}

JavaWebsocket.prototype.send = function (uint8Array) {
    let ss = [];
    for (let i = 0; i < uint8Array.length; i++) {
        ss.push(uint8Array[i]);
    }
    jsb.reflection.callStaticMethod("sockets/WebSocketClient",
        "sendPacket", "(Ljava/lang/String;)V", ss.join("|"));
}

JavaWebsocket.prototype.disconnect = function () {
    this.websocket.readyState = WS_CLOSING;
    jsb.reflection.callStaticMethod("sockets/WebSocketClient",
        "disconnect", "()V");
    window["wshandler"]["websocket"] = null;
}
...
window["wshandler"] = {
    ...
    event: function (event, strdata) {
        cc.log("[pomelo-client-android] 收到消息", event);
        switch (event) {
            case "onopen": {
                if (this.websocket) {
                    this.websocket.readyState = WS_OPEN;
                    this.websocket.onopen({ target: this.websocket, data: strdata })
                } else {
                    cc.error(" websocket is null!")
                }
                break;
            }
            case "onmessage": {
                if (this.websocket) {
                    ...
                    cc.log("[pomelo-client-android] 消息size", ss.length);

                    let bytes = new Uint8Array(ss.length);
                    for (let i = 0; i < ss.length; i++) {
                        bytes[i] = parseInt(ss[i]);
                    }
                    this.websocket.onmessage({ target: this.websocket, data: bytes })
                } else {
                    cc.error(" websocket is null!")
                }
                break;
            ...
            case "onclose": {
                if (this.websocket) {
                    ...
                    this.websocket.onclose({ target: this.websocket, data: strdata })
                } else {
                    cc.error(" websocket is null!")
                }
                break;
            ...
            case "onerror": {
                if (this.websocket) {
                    this.websocket.onerror({ data: strdata })
                } else {
                    cc.error(" websocket is null!")
                }
                break;
...
let pomeloCreator = function (protoLocalkey) {
    ...
    pomelo.init = function (params, cb, eb) {
        ...
        let url = (params.ssl ? 'wss://' : 'ws://') + host;
    ...
    pomelo.disconnect = function () {
        try {
            if (socket) {
                ...
                cc.log("【pomelo】disconnect 销毁socket");
                socket = null;
            }
    ...
    let send = function (packet) {
        cc.log("socket", socket.readyState);
